...
function MapPort(props) {
  ...
  const markerCityInfo = () => {
    ...
    if (resultPorts.type === "city") {
      ...
      if (latlng.latTo === resultPorts.cityInfo.lat && latlng.lngTo === resultPorts.cityInfo.lng) {
  ...
  const markerPorts = resultPorts.seaPorts.map(location => {
    ...
    let markerDefault = markerIconDefault;
    if (latlng.latTo === location.lat && latlng.lngTo === location.lng) {
      markerDefault = markerIconHover;
    }
    return (
      <Marker
  ...
  return (
    <div>
      <Map style={{ width: "408px", height: "314px", borderRadius: "10px" }} center={[latlng.latTo, latlng.lngTo]} zoom={6} scrollWheelZoom={true}>
        <TileLayer
